...
class TemporalFeatureCombiner:
    ...
    def combine(self):
        ...
        raw_feature_cols = [
            'trend_macd', 'trend_ema_50', 'trend_ema_100', 'trend_macd_signal',
            'volume_obv', 'volume_cmf', 'volume_vpt',
            'lag_14', 'diff_14', 'trend_sma_100', 'trend_ema_250',
            'volume_ad', 'lag_35', 'diff_35', 'trend_sma_300', 'trend_ema_1000',
            'trend_sma_650', 'volatility_donchian_high_45', 'volatility_donchian_low_45',
            'lag_100', 'diff_100'
        ]
        ...
        df['TI_short_lag_movement'] = (
            0.5 * df.get('lag_14', 0) +
            0.5 * df.get('diff_14', 0)
        )
        ...
        df['TI_medium_lag_movement'] = (
            0.5 * df.get('lag_35', 0) +
            0.5 * df.get('diff_35', 0)
        )
        ...
        df['TI_long_donchian_channel_width'] = (
            df.get('volatility_donchian_high_45', 0) - df.get('volatility_donchian_low_45', 0)
        )

        df['TI_long_lag_signal'] = (
            0.5 * df.get('lag_100', 0) +
            0.5 * df.get('diff_100', 0)
        )
